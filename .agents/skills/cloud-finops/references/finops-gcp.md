...
**Suboptimal GCS Storage Class Tiering And Lifecycle Management**
Service: GCP GCS | Type: Inefficient Configuration

Google Cloud Storage (GCS) provides 4 primary storage classes: Standard (frequent access), Nearline (accessed < once/month, 30-day min), Coldline (accessed < once/quarter, 90-day min), and Archive (accessed < once/year, 365-day min). 
FinOps best practices:
- Configure Object Lifecycle Management (OLM) rules to automatically transition objects from Standard to Nearline (after 30 days), Coldline (after 90 days), and Archive (after 365 days) based on age.
- For unpredictable access patterns, enable GCS Autoclass to automatically shift objects across Nearline, Coldline, and Archive without retrieval fees.
- Account for minimum storage duration penalties: deleting or replacing Nearline objects before 30 days, Coldline before 90 days, or Archive before 365 days incurs early deletion charges.
